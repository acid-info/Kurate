import { connectWallet, getGlobalAnonymousFeed, getProvider } from "@/services";
import { type Chat } from "@/interfaces/Chat";
import { Persona, type DraftPersona } from "@/interfaces/Persona";
...
import { getFromLocalStorage, saveToLocalStorage, sleep } from "@/utils";
...
export class ZkitterAdapter implements Adapter {
  ...
  async start() {
    ...
    const { addApproved, addPending } = usePostContext();
    const { chatData, updateChatData } = useChatContext();

    zkitter.on(
      "Zkitter.NewMessageCreated",
      async (msg: Message, proof: Proof) => {
        if (msg.type === "POST") {
          ...
          if (
            protocol !== "kurate" &&
            contractAddress !== GLOBAL_ANONYMOUS_FEED_ADDRESS
          )
            return;
            ...
            addApproved(
              {
                ...
                images: post.payload.attachment
        ...
        if (msg.type === "CHAT") {
          ...
          const { pub, priv } = await generateECDHKeyPairFromZKIdentity(
            zkIdentity,
            postId
          );
    ...
    updateChatData({ ...chatData, loading: false });
  }
  ...
  private async syncActivePost(personaId: string) {
    const { addApproved } = usePostContext();
    const contract = getGlobalAnonymousFeed();
    const activePosts = await contract.queryFilter(
      contract.filters.NewPersonaMessage(personaId)
    );
  ...
  private async queryPersonasFromContract(contract: GlobalAnonymousFeed) {
    ...
    for (let i = 0; i < numOfPersonas; i++) {
      ...
      if (!personaData.all.has(personaId)) {
    ...
    updatePersonaData({ ...personaData, loading: false });
  }
  ...
  private async syncPersonaData(contract: GlobalAnonymousFeed) {
    ...
    for (let i = 0; i < numOfPersonas; i++) {
      ...
      updatedPersonaData.all.set(personaId, persona);
    }

    updatePersonaData(updatedPersonaData);
  }

  async queryPersonaJoined(personaId: string): Promise<boolean> {
    const identityCommitment =
      this.identity?.unirepIdentity.genIdentityCommitment();

    if (!identityCommitment) return false;

    const zkitter = await this.getZkitterClient();

    const members = await zkitter.getGroupMembers(
      GroupAdapter.createGroupId(personaId)
    );
  ...
  private async loadFavorite(): Promise<void> {
    ...
    if (favorite.length) {
      await zkitter.updateFilter({
        group: favorite.map(GroupAdapter.createGroupId),
      } as never);

      updatePersonaData({ ...personaData, favorite });
    }
  }
  ...
  addPersonaToFavorite(groupId: string): Promise<void> {
    return new Promise((resolve) => {
      ...
      const favoriteNew: string[] = [...personaData.favorite, groupId];

      saveToLocalStorage("favorite", favoriteNew);

      resolve();
      updatePersonaData({ ...personaData, favorite: favoriteNew });
    });
  }
  ...
  removePersonaFromFavorite(groupId: string): Promise<void> {
    return new Promise((resolve) => {
      ...
      const favoriteNew = personaData.favorite.filter((s) => s !== groupId);
  ...
  async publishPersona(draftPersona: DraftPersona): Promise<string> {
    ...
    const { MessageType, Post, PostMessageSubType } = await import(
      "zkitter-js"
    );
    ...
    if (draftPersona.posts.length !== 5)
      throw new Error("must contain exactly 5 seed posts");
    if (!draftPersona.picture)
      throw new Error("must contain a profile picture");
    ...
    const resp = await fetch(`${RELAYER_URL}/create-and-join-with-rep`, {
      ...
      body: JSON.stringify({
        ...
        seedPostHashes: seedPostHashes as [
          string,
          string,
          string,
          string,
  ...
  async publishPost(
    ...
    if (!this.identity) throw new Error("must sign in first");
    const { Post, MessageType, PostMessageSubType } = await import(
      "zkitter-js"
    );
    const zkitter = await this.getZkitterClient();
    const { addPending } = usePostContext();
  ...
  private async userStateTransition(
    ustProof: UserStateTransitionProof
  ): Promise<string> {
  ...
  async startChat(chat: Chat): Promise<string> {
    ...
    const { generateECDHKeyPairFromZKIdentity, deriveChatId } = await import(
      "zkitter-js"
    );
    const { pub } = await generateECDHKeyPairFromZKIdentity(
      zkIdentity,
      chat.post.postId
    );
    const { ecdh: receiverECDH } = msgProof;
    ...
    updatedChatData.chats.set(chatId, {
      ...chat,
      chatId,
    });
  ...
  async sendChatMessage(chatId: string, text: string): Promise<void> {
    ...
    const chatInfo = chatData.chats.get(chatId);
  ...
  async subscribeToChat(chatId: string): Promise<() => unknown> {
    ...
    const chatMsgs = await zkitter.getChatMessages(
      chatId,
      undefined,
      undefined,
      {
  ...
  private insertChat(chat: ZKitterChat, chatId: string) {
    ...
    if (!chatStore?.messages.find(({ messageId }) => messageId === chatHash)) {
      chatStore?.messages.push({
        ...
        text:
          typeof chat.payload.content === "undefined"
            ? "encryption error"
