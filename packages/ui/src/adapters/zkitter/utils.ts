...
export async function createIdentity(
  ...
  const { generateP256FromSeed, generateZKIdentityWithP256 } = await import(
    "zkitter-js"
  );
...
export async function generateRLNProofForNewPersona(
  ...
  const { createRLNProof, generateECDHKeyPairFromZKIdentity } = await import(
    "zkitter-js"
  );
...
export async function updateActivePosts(
  postHashes: string[]
): Promise<{ [hash: string]: string }> {
  const actives = getFromLocalStorage<{ [hash: string]: string }>(
    "kurate/actives",
    {}
  );
  const newActives = {
